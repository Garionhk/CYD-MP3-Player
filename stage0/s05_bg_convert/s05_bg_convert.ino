// ===========================================================================
// Stage 0.5 -- backgrounds converted once at boot, then played beside MP3/BT
// ===========================================================================
// s04 showed live GIF decoding leaves ~10 KB of heap once Bluetooth is up:
// AnimatedGIF's ~25 KB of LZW state is simply too much to hold alongside the
// Bluetooth stack and the MP3 decoder. So the decoder never meets Bluetooth:
//
//   BOOT (no Bluetooth yet, ~155 KB free)
//     for each /bg/bgN.gif whose /.sys/bgN.anim is missing or stale:
//       decode with AnimatedGIF, scale/crop to 240x240 or 240x320,
//       write the pixels each frame changes to /.sys/bgN.anim
//     free the decoder
//   PLAYBACK (Bluetooth up)
//     read spans from .anim and push them to the panel -- ~2 KB of RAM
//
// Size rule (decided with the owner): nothing is rejected. The GIF is scaled
// to COVER the target and centre-cropped. Near-square -> 240x240; tall
// (height >= 1.17 x width) -> 240x320, centre-cropped to 240x240 on this
// landscape layout.
//
// .anim format (little-endian):
//   header  "CANM" u8 ver=1 u8 0 u16 w u16 h u16 frames u32 srcSize u32 srcTime
//   frame   u16 delayMs, then spans: u16 y u16 x u16 n, n x u16 RGB565 (panel
//           byte order), terminated by y = 0xFFFF
//
// Screen: same layout and controls as s04. Serial adds conversion timing and
// the playback cost of each background.
// ===========================================================================
...
// ---------------------------------------------------------------------------
// Background converter (boot only, before Bluetooth)
// ---------------------------------------------------------------------------
static const uint16_t ANIM_END_OF_FRAME = 0xFFFF;
static const uint32_t ANIM_HEADER_LEN   = 20;
...
// State shared between convertGif() and the draw callback.
struct Conv {
  File      in, out;
  int       tw, th;                 // target canvas
...
static void convWrite(const void* data, int len) {
  ...
  while (len > 0) {
    const int n = min(len, (int)sizeof(conv.wbuf) - conv.wlen);
    memcpy(conv.wbuf + conv.wlen, p, n);
    conv.wlen += n; p += n; len -= n;
    if (conv.wlen == sizeof(conv.wbuf)) {
...
static int32_t convReadCb(GIFFILE* pf, uint8_t* buf, int32_t len) {
  ...
  int32_t n = len;
  if (pf->iSize - pf->iPos < len) n = pf->iSize - pf->iPos - 1;   // library quirk
  if (n <= 0) return 0;
  n = f->read(buf, n);
  pf->iPos = f->position();
  return n;
...
static void convDrawCb(GIFDRAW* d) {
  ...
  for (int r = 0; r < rows; r++) {
    ...
    int tx = 0;
    while (tx < conv.tw) {
      // Skip pixels this frame does not paint: outside its rectangle, or transparent.
      while (tx < conv.tw) {
        const int sx = conv.xmap[tx] - d->iX;
        if (sx >= 0 && sx < d->iWidth &&
            !(d->ucHasTransparency && s[sx] == d->ucTransparent)) break;
        tx++;
      }
      const int start = tx;
      int n = 0;
      while (tx < conv.tw) {
        const int sx = conv.xmap[tx] - d->iX;
        if (sx < 0 || sx >= d->iWidth) break;
        if (d->ucHasTransparency && s[sx] == d->ucTransparent) break;
        line[n++] = d->pPalette[s[sx]];
        tx++;
      }
      if (n) {
...
// Returns true when /.sys/bgN.anim is valid for the current bgN.gif.
static bool animIsCurrent(const char* animPath, File& src) {
  ...
  const bool ok = a.read((uint8_t*)&h, sizeof(h)) == sizeof(h) &&
                  memcmp(h.magic, "CANM", 4) == 0 && h.ver == 1 && h.frames > 0 &&
...
static void convScreen(int n, int total, const String& line2) {
  tft.fillRect(0, 90, 320, 60, TFT_BLACK);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("Preparing background " + String(n) + " / " + String(total), 160, 105, 2);
  tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
  tft.drawString(line2, 160, 130, 2);
}

static bool convertGif(int n, int total) {
  ...
  if (!g) { Serial.println("conv: no memory for decoder"); return false; }
  ...
  if (!g->open(gifPath, convOpenCb, convCloseCb, convReadCb, convSeekCb, convDrawCb)) {
    Serial.printf("conv: %s open failed (err %d)\n", gifPath, g->getLastError());
    delete g;
    return false;
  }
  const int sw = g->getCanvasWidth(), sh = g->getCanvasHeight();
  conv.tw = 240;
  conv.th = (sh * 100 >= sw * 117) ? 320 : 240;
  // Cover: scale so both dimensions reach the target, then centre-crop.
  const float scale = max((float)conv.tw / sw, (float)conv.th / sh);
  ...
  AnimHeader h = { {'C','A','N','M'}, 1, 0, (uint16_t)conv.tw, (uint16_t)conv.th, 0, srcSize, srcTime };
  convWrite(&h, sizeof(h));

  Serial.printf("conv: %s %dx%d -> %dx%d (scale %.3f)  heap %u\n", gifPath, sw, sh,
                conv.tw, conv.th, scale, ESP.getFreeHeap());
  int frames = 0, delayMs = 0, rc = 1;
  while (rc > 0 && !conv.failed && frames < 1000) {
    // Frame header goes first; the draw callback appends this frame's spans
    // during playFrame, and the terminator closes it. The delay is only known
    // after the frame is decoded, so it is patched in via a placeholder.
    convFlush();
    const uint32_t frameAt = conv.bytes;
    const uint16_t placeholder = 0;
    convWrite(&placeholder, 2);
    rc = g->playFrame(false, &delayMs);
    const uint16_t end = ANIM_END_OF_FRAME;
    convWrite(&end, 2);
    convFlush();
    ...
    if (rc < 0) break;
    frames++;
    if (frames % 5 == 0) convScreen(n, total, "frame " + String(frames));
  }
  const int err = g->getLastError();
  g->close();
  delete g;
  free(conv.xmap); free(conv.rowFirst); free(conv.rowCount);

  if (frames == 0 || conv.failed) {
    conv.out.close();
    SD.remove(tmpPath);
    Serial.printf("conv: %s FAILED after %d frames (gif err %d, write fail %d)\n",
  ...
  Serial.printf("conv: %s -> %s  %d frames  %u KB  %lu ms  heap after %u\n", gifPath,
                animPath, frames, conv.bytes / 1024, millis() - t0, ESP.getFreeHeap());
  return true;
}

static int bgCount = 0;

static void convertBackgrounds() {
  char path[24];
  for (int i = 1; i < 100; i++) {
  ...
  for (int i = 1; i <= bgCount; i++) {
    convScreen(i, bgCount, "checking...");
    convertGif(i, bgCount);
  }
  tft.fillScreen(TFT_BLACK);
}

// ---------------------------------------------------------------------------
// Background player (.anim, runs beside Bluetooth)
// ---------------------------------------------------------------------------
static File     anim;
static bool     animOpen = false;
static int      bgIndex = 1;               // 0 = off
static int      animOffY = 0;
static uint32_t animNextAt = 0;
static uint32_t animFrames = 0, animDrawMs = 0, animDeferred = 0;
static uint8_t  rbuf[1024];                // read buffer over the .anim file
static int      rpos = 0, rlen = 0;

static const int AREA_W = 240, AREA_H = 240;
static const int FPS_CAP = 15;

struct Rect { int x, y, w, h; };
static const Rect MASKS[] = { { 0, 204, 240, 36 } };   // title band
static const int MASK_COUNT = sizeof(MASKS) / sizeof(MASKS[0]);

static bool animRead(void* dst, int len) {
...
static void animRewind() {
  anim.seek(ANIM_HEADER_LEN);
  rpos = rlen = 0;
}
...
static void animClose() {
  if (animOpen) anim.close();
  animOpen = false;
}

static bool animStart(int n) {
  animClose();
  tft.fillRect(0, 0, AREA_W, 204, TFT_BLACK);
  if (n <= 0) return false;
  char path[24];
  ...
  if (!anim || anim.read((uint8_t*)&h, sizeof(h)) != sizeof(h) ||
    ...
    if (anim) anim.close();
    Serial.printf("anim: %s missing or invalid\n", path);
    return false;
  }
  animOffY = (AREA_H - h.h) / 2;           // 320 tall -> -40 centre crop
  rpos = rlen = 0;
  animOpen = true;
  animNextAt = 0;
  Serial.printf("anim: %s %ux%u %u frames  heap -%d\n", path, h.w, h.h, h.frames,
                (int)(h0 - ESP.getFreeHeap()));
  return true;
}

static void animTick(uint32_t now) {
  if (!animOpen || now < animNextAt) return;
  ...
  const uint32_t t0 = millis();
  uint16_t delayMs;
  if (!animRead(&delayMs, 2)) {            // end of file: loop
    animRewind();
    if (!animRead(&delayMs, 2)) { animClose(); return; }
  }
  static uint16_t line[240];
  tft.startWrite();
  for (;;) {
    uint16_t hdr[3];
    if (!animRead(hdr, 2)) break;
    if (hdr[0] == ANIM_END_OF_FRAME) break;
    if (!animRead(hdr + 1, 4)) break;
    const int n = min<int>(hdr[2], 240);
    if (!animRead(line, n * 2)) break;
    const int y = hdr[0] + animOffY;
    if (y >= 0 && y < AREA_H) pushSpanMasked(y, hdr[1], hdr[1] + n, line);
  }
  tft.endWrite();
  animFrames++;
  animDrawMs += millis() - t0;
  animNextAt = t0 + max<int>(delayMs, 1000 / FPS_CAP);
}
...
static void nextBackground() {
  for (int tries = 0; tries <= bgCount; tries++) {
    ...
    if (bgIndex == 0) { animStart(0); return; }
    if (animStart(bgIndex)) return;
  }
}
...
void setup() {
  Serial.begin(115200);
  delay(300);
  Serial.println("\n=== s05 background converter + MP3 to Bluetooth ===");
  cydPrintBanner();
  ...
  convertBackgrounds();                     // BEFORE Bluetooth -- see header
...
void loop() {
  ...
  if (connected) {
    if (!shownPlayer || trackChanged) {
      ...
      if (!shownPlayer) {
        tft.fillScreen(TFT_BLACK);
        shownPlayer = true;
        if (bgIndex && !animOpen && !animStart(bgIndex)) nextBackground();
      }
      drawButtons();
      drawBand();
    }
    ...
    if (shownPlayer) { shownPlayer = false; animClose(); listDirty = true; }
  ...
  if (now - lastLog >= 5000) {
    ...
    Serial.printf("stat: heap %u min %u largest %u  dec-stack %u  frames/s %u  underruns %u  "
                  "ring %u%%  bg %s fps %u.%u avg %ums deferred %u  %s\n",
                  ...
                  animOpen ? ("bg" + String(bgIndex)).c_str() : "off",
                  animFrames * 1000 / dt, (animFrames * 10000 / dt) % 10,
                  animFrames ? animDrawMs / animFrames : 0, animDeferred,
                  connected ? (playing ? "playing" : "paused") : "not connected");
    animFrames = animDrawMs = animDeferred = 0;
    lastFrames = f;
    lastLog = now;
  }
  delay(1);
