...
# See build_all.sh: min_spiffs gives the app its two images (player + uploader).
# UploadSpeed=115200: this board's CH340 drops the switch to 921600 ("Failed
# uploading: exit status 2") -- measured on the first s01 flash.
FQBN="${FQBN:-esp32:esp32:esp32:PartitionScheme=min_spiffs,UploadSpeed=115200}"
# app1 in min_spiffs.csv. arduino-cli upload writes the player to app0; the
# uploader image goes here with esptool.
UPLOADER_OFFSET=0x1F0000
BAUD="${BAUD:-115200}"
...
if [ "$SKETCH" = "app" ]; then
  echo
  echo "--- building and flashing the uploader image (app1 @ $UPLOADER_OFFSET) ---"
  UPDIR="$ROOT/app/build/uploader"
  "$CLI" compile --fqbn "$FQBN" \
      ...
      --build-path "$ROOT/app/build/uploader-cache" --output-dir "$UPDIR" "$SKETCH" || exit 1
  ...
  "$ESPTOOL" --port "$PORT" --baud 115200 write-flash "$UPLOADER_OFFSET" "$UPDIR/app.ino.bin" || {
    echo "Uploader flash failed -- the player works, but Setup > Upload music will say it is missing." >&2
    exit 1
  }
fi
